...
<div id="app" >
    <nav class="navbar navbar-expand-md navbar-light bg-white shadow-sm">
        <div class="container">
            <a style="color: tomato;" class="navbar-brand" href="{{ url('/') }}">
                {{ env('APP_NAME') }} 
                        ...
                        @else
                            <li class="nav-item dropdown">
                                <a style="color: green;" id="navbarDropdown" class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                    Active Users <span class="caret"></span>
                                </a>
                                    ...
                                    @foreach($users as $user)
                                        @if($user->isOnline())
                                            <div> 
                                                ...
                                                <a href="{{route('live_chat')}}">{{$user->name}}</a>
                                            </div> <hr>
                                        @endif
                                    @endforeach
                                <div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.user').click(function () {
            ...
            $.ajax({
                type: "get",
                url: "live_chat_show/" + receiver_id, // need to create this route
                data: "",
                cache: false,
        ...
        $(document).on('keyup', '.input-text input', function (e) {
            ...
            if (e.keyCode == 13 && message != '' && receiver_id != '') {
                ...
                document.getElementById('ChatAudio').play(); //message send with playing tone

                var datastr = "receiver_id=" + receiver_id + "&message=" + message;
                $.ajax({
                    type: "post",
                    url: "live_chat_show", // need to create this post route
                    data: datastr,
                    cache: false,
                    success: function (data) {
